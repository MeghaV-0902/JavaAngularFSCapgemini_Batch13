...
-- desc newAuthor;
-- insert into newAuthor values(1,"Megha","India","Blore");
...
select custid,fname,dob from customer order by dob desc;

use ManifacturingProducts;
update Products set Price=0.5*Price*20 where Price>0;
select * from Products;
Update Products set  Price  = Price-0.6*Price  where price >120;

update Products set name = "Laser Printer" where name="Printer";

use mydb;
create table stud(st_id integer primary key,course varchar(30), name varchar(30));
select * from stud;
